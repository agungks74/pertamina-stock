import { type User, type InsertUser, type DashboardStatus, type RegionStatus, type InsertDashboardStatus, type InsertRegionStatus } from "@shared/schema";
...
export interface IStorage {
  ...
  getDashboardStatuses(): Promise<DashboardStatus[]>;
  getRegionStatuses(): Promise<RegionStatus[]>;
  createDashboardStatus(status: InsertDashboardStatus): Promise<DashboardStatus>;
  createRegionStatus(status: InsertRegionStatus): Promise<RegionStatus>;
}

export class MemStorage implements IStorage {
  private users: Map<string, User>;
  private dashboardStatuses: Map<string, DashboardStatus>;
  private regionStatuses: Map<string, RegionStatus>;

  constructor() {
    this.users = new Map();
    this.dashboardStatuses = new Map();
    this.regionStatuses = new Map();
    this.initializeData();
  }
